...
func filterAndFormat(input string, cutout ...string) string {
	output := input
	for _, c := range cutout {
	...
	filterPattern := `(\ {2,})`
	regexFilter := regexp.MustCompile(filterPattern)
	output = fmt.Sprintf("%s", regexFilter.ReplaceAllLiteralString(output, " "))

	formaterPattern := `(\ \,)`
	regexFormater := regexp.MustCompile(formaterPattern)

	output = regexFormater.ReplaceAllLiteralString(output, ",")
	output = strings.TrimFunc(output, unicode.IsSpace)
	return output
}
